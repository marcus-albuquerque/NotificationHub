...
export class EventHistory {
   ...
   */
  async save(entry: EventHistoryEntry): Promise<EventHistoryEntry> {
    const pool = getPool();
    const client = await pool.connect();

    try {
      // Generate ID if not provided
      const historyId = entry.id || uuidv4();
      ...
      throw new Error(
        `Failed to save event history entry: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
   ...
   * @throws Error if database operation fails
   */
  async getByEventId(eventId: string): Promise<EventHistoryEntry | null> {
    const pool = getPool();
    const client = await pool.connect();
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve event history by eventId: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
  ...
  async getByFarmId(
    ...
    limit: number = 20,
    offset: number = 0
  ): Promise<{ entries: EventHistoryEntry[]; total: number }> {
    const pool = getPool();
    const client = await pool.connect();
    ...
    try {
      ...
      const entries = result.rows.map((row: any) => this.mapRowToEntry(row));
      return { entries, total };
    } catch (error) {
      throw new Error(
        `Failed to retrieve event history by farmId: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
  ...
  async getByDeviceId(
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve event history by deviceId: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
  ...
  async getByDateRange(
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve event history by date range: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
  ...
  async getByValidationStatus(
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve event history by validation status: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
  ...
  async getDuplicates(
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve duplicate events: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      client.release();
    }
   ...
   */
  private mapRowToEntry(row: any): EventHistoryEntry {
    return {
      id: row.history_id,
