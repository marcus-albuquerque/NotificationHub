...
export class DemoDataLoader {
   ...
   */
  private async createDemoProducer(): Promise<void> {
    ...
    await this.pool.query(
      ...
      [producerId, name, phone, email, notificationPreferences]
    );
  }
   ...
   */
  private async createDemoFarm(): Promise<void> {
   ...
   */
  private async createDemoDevices(): Promise<void> {
    const farmId = 'farm-001';
    ...
    for (const device of devices) {
      ...
      await this.pool.query(
        ...
        [device.deviceId, farmId, device.name, device.sensorType]
      );
    }
  }
   ...
   */
  private async generateTriggeringEvents(): Promise<void> {
    const farmId = 'farm-001';

    const triggeringEvents = [
      {
        eventId: `evt-trigger-${uuidv4().substring(0, 8)}`,
        deviceId: 'temp-001',
        sensorType: SensorType.AIR_TEMPERATURE,
        value: '36.5',
        unit: '°C',
    ...
    for (const event of triggeringEvents) {
      ...
      const existing = await this.pool.query(
        'SELECT * FROM events WHERE device_id = $1 AND value = $2 AND sensor_type = $3 AND event_id LIKE $4',
        [event.deviceId, event.value, event.sensorType, 'evt-trigger-%']
      );
   ...
   */
  private async generateNormalEvents(): Promise<void> {
    const farmId = 'farm-001';

    const normalEvents = [
      {
        eventId: `evt-normal-${uuidv4().substring(0, 8)}`,
        deviceId: 'temp-001',
        sensorType: SensorType.AIR_TEMPERATURE,
        value: '25.0',
        unit: '°C',
    ...
    for (const event of normalEvents) {
      ...
      const existing = await this.pool.query(
        'SELECT * FROM events WHERE device_id = $1 AND value = $2 AND sensor_type = $3 AND event_id LIKE $4',
        [event.deviceId, event.value, event.sensorType, 'evt-normal-%']
      );
 ...
 */
let demoDataLoaderInstance: DemoDataLoader | null = null;

export function getDemoDataLoader(): DemoDataLoader {
