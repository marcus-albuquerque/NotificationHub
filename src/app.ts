...
import { createHistoryRouter } from './routes/historyRoutes';
...
export function createApp(): Express {
    ...
    try {
      ...
      const insertResult = await pool.query(
        `INSERT INTO farms (farm_id, name, producer_id, created_at, updated_at)
         ...
         RETURNING farm_id, name, producer_id, created_at, updated_at`,
        [farmId, name, producerId]
      );
   ...
   */
  app.get('/api/farms/:farmId', async (req: Request, res: Response): Promise<void> => {
    try {
   ...
   */
  app.get('/api/farms/:farmId/devices', async (req: Request, res: Response) => {
    try {
      const { farmId } = req.params;
      const pool = getPool();
      const result = await pool.query(
        'SELECT * FROM devices WHERE farm_id = $1 ORDER BY sensor_type, name',
        [farmId]
      );
  ...
  app.get(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        const pool = getPool();
        const result = await pool.query(
          'SELECT * FROM devices WHERE device_id = $1 AND farm_id = $2',
          [deviceId, farmId]
        );
    ...
    try {
      ...
      const insertResult = await pool.query(
        `INSERT INTO producers (producer_id, name, phone, email, notification_preferences, created_at, updated_at)
         ...
         RETURNING producer_id, name, phone, email, notification_preferences, created_at, updated_at`,
        [producerId, name, phone, email || null, JSON.stringify(prefs)]
      );
  ...
  app.get('/api/producers', async (req: Request, res: Response) => {
    try {
      ...
      // Get paginated results
      const result = await pool.query(
        `SELECT producer_id, name, phone, email, notification_preferences, created_at, updated_at
         FROM producers
         ORDER BY name
         LIMIT $1 OFFSET $2`,
        [limit, offset]
      ...
      const producers = result.rows.map((row: any) => ({
        producer_id: row.producer_id,
        name: row.name,
        phone: row.phone,
        email: row.email,
   ...
   */
  app.get('/api/producers/:producerId', async (req: Request, res: Response): Promise<void> => {
    try {
   ...
   */
  app.get('/api/farms/:farmId/notifications', async (req: Request, res: Response) => {
    try {
      const { farmId } = req.params;
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 20;
      ...
      // Optional filters
      const ruleName = req.query.ruleName as string;
