...
// Queue instance
let notificationRetryQueue: BullQueue<Notification> | null = null;
 ...
 */
export async function initializeQueue(): Promise<BullQueue<Notification>> {
  logger.info('Initializing notification retry queue...', {
  ...
  notificationRetryQueue = new Queue<Notification>('notification-retry', {
    redis: {
      host: REDIS_HOST,
      port: REDIS_PORT,
  ...
  await notificationRetryQueue.process(QUEUE_CONCURRENCY, async (job: Job<Notification>) => {
    logger.info(`Processing notification retry job ${job.id}`, {
   ...
   */
  notificationRetryQueue.on('completed', (job: Job<Notification>) => {
   ...
   */
  notificationRetryQueue.on('failed', (job: Job<Notification>, error: Error) => {
   ...
   */
  notificationRetryQueue.on('stalled', (job: Job<Notification>) => {
    logger.warn(`Notification retry job stalled`, {
      jobId: job.id,
      notificationId: job.data.notificationId,
      attempts: job.attemptsMade + 1,
    });
   ...
   */
  notificationRetryQueue.on('error', (error: Error) => {
  ...
  notificationRetryQueue.on('ready', () => {
    logger.info('Notification retry queue is ready for processing');
  });
  ...
  notificationRetryQueue.on('paused', () => {
    logger.warn('Notification retry queue has been paused');
  });
  ...
  notificationRetryQueue.on('resumed', () => {
    logger.info('Notification retry queue has been resumed');
  });

  logger.info('Notification retry queue initialized successfully');
 ...
 */
export function getQueue(): BullQueue<Notification> | null {
  return notificationRetryQueue;
}
 ...
 */
export async function shutdownQueue(timeout: number = 10000): Promise<void> {
  if (!notificationRetryQueue) {
    logger.info('Queue not initialized, skipping shutdown');
    return;
  }

  logger.info('Shutting down notification retry queue...', { timeout });

  try {
    // Pause the queue to stop processing new jobs
    await notificationRetryQueue.pause();
    logger.info('Queue paused, waiting for in-progress jobs to complete...');

    // Close queue
    await notificationRetryQueue.close();
    logger.info('Notification retry queue shut down successfully');
...
export async function clearQueue(): Promise<void> {
  ...
  logger.warn('Clearing all jobs from notification retry queue');
