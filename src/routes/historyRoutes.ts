...
import { ApiResponse, EventHistoryEntry } from '../types/index';
...
export function createHistoryRouter(): Router {
  ...
  router.get('/search', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const isDuplicateStr = req.query.isDuplicate as string;
      ...
      if (startDate && endDate) {
        ...
        result = await eventHistory.getByDateRange(farmId, startDate, endDate, limit, offset);
      } else if (status) {
        // Status-based query
        if (!farmId) {
          const response: ApiResponse<null> = {
        ...
        result = await eventHistory.getByValidationStatus(
          farmId,
          status as 'valid' | 'rejected',
          limit,
          offset
        );
        ...
        if (isDuplicate) {
          result = await eventHistory.getDuplicates(farmId, limit, offset);
        } else {
          // Query non-duplicate events (requires farm-based query)
          result = await eventHistory.getByFarmId(farmId, limit, offset);
        }
      } else {
        ...
        if (!farmId) {
          const response: ApiResponse<null> = {
            success: false,
            error:
              'At least one filter parameter (farmId, date range, status, or isDuplicate) is required',
            timestamp: new Date().toISOString(),
          };
          res.status(400).json(response);
          return;
      ...
      if (farmId) appliedFilters.farmId = farmId;
      ...
      if (isDuplicate !== undefined) appliedFilters.isDuplicate = isDuplicate;

      const response: ApiResponse<EventHistoryEntry[]> = {
        success: true,
        data: result.entries,
        timestamp: new Date().toISOString(),
      };
  ...
  router.get('/:eventId', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const entry = await eventHistory.getByEventId(eventId);
   ...
   */
  router.get('/farm/:farmId', async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  router.get('/device/:deviceId', async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Query by device ID
      const result = await eventHistory.getByDeviceId(deviceId, limit, offset);
