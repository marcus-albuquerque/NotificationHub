...
class RuleEngine {
    ...
    initializeBuiltInRules() {
        const builtInRules = [
            {
                ...
                sensorType: index_1.SensorType.AIR_TEMPERATURE,
                condition: (value) => {
            ...
            },
            {
                ...
                sensorType: index_1.SensorType.AIR_HUMIDITY,
                condition: (value) => {
                    const numValue = typeof value === "number" ? value : parseFloat(value);
            ...
            },
            {
                ...
                sensorType: index_1.SensorType.SOIL_MOISTURE,
                condition: (value) => {
                    const numValue = typeof value === "number" ? value : parseFloat(value);
            ...
            },
            {
                ...
                sensorType: index_1.SensorType.WATER_RESERVOIR_LEVEL,
                condition: (value) => {
                    const numValue = typeof value === "number" ? value : parseFloat(value);
            ...
            },
            {
                ...
                name: "Low Silo Level",
                sensorType: index_1.SensorType.SILO_LEVEL,
                condition: (value) => {
                    const numValue = typeof value === "number" ? value : parseFloat(value);
                    return !isNaN(numValue) && numValue < 15;
                },
            ...
            },
            {
                ...
                sensorType: index_1.SensorType.EQUIPMENT_STATUS,
                condition: (value) => {
        ...
        logger_1.logger.info(`RuleEngine initialized with ${builtInRules.length} built-in rules`);
    }
    /**
     ...
     */
    getRulesBySensorType(sensorType) {
        ...
        for (const rule of applicableRules) {
            try {
                ...
                if (conditionMet) {
                    ...
                    logger_1.logger.debug(`Rule fired: ${rule.ruleId} for event ${event.eventId} with value ${event.value}`);
                }
            }
            catch (error) {
                logger_1.logger.error(`Error evaluating rule ${rule.ruleId} for event ${event.eventId}: ${error instanceof Error ? error.message : String(error)}`);
                // Don't add this rule to firedRules if condition evaluation fails
                // This prevents a buggy condition from causing issues
            }
        }
        if (firedRules.length === 0) {
            logger_1.logger.debug(`No rules fired for event ${event.eventId} (sensorType: ${event.sensorType})`);
        }
        return firedRules;
    }
    /**
    ...
    registerRule(rule) {
        ...
        this.rules.set(rule.ruleId, rule);
        logger_1.logger.info(`Custom rule registered: ${rule.ruleId}`);
    }
    /**
    ...
    updateRule(rule) {
        ...
        this.rules.set(rule.ruleId, rule);
        logger_1.logger.info(`Rule updated: ${rule.ruleId}`);
    }
    /**
    ...
    removeRule(ruleId) {
        ...
        if (result) {
            logger_1.logger.info(`Rule removed: ${ruleId}`);
        }
        return result;
    }
    /**
